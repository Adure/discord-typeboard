...
async def on_typing(channel, user, time):
    ...
    try:
        ...
        print("{0.name}#{0.discriminator} didn\'t send a message".format(user))
    ...
    print("{0.author.name}#{0.author.discriminator} took {1} seconds to send the message \"{2}\"".format(msg, durationTime, formattedMsg))
